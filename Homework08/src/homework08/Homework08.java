...
class MyMatrix {
	...
	private int sequentialSearch(int key, int x, int y, int count){
		if(key == A[y][x]) //if key is found
			return count;
		if(x == size-1 && y == size-1 && key != A[y][x]) //if at the end of the array and key is not found
			return -1;
		else {
			if(x == size-1) //if at the end of the row
				return sequentialSearch(key, 0, ++y, ++count);
			else 
				return sequentialSearch(key, ++x, y, ++count);
		}
	}
